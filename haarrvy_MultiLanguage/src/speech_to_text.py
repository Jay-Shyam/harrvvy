...
genai.configure(api_key="AIzaSyDPSvth3r5uKUo8C-wQbI2L1bhomZsheH8")
...
def transcribe_audio(file_path: str) -> str:
    ...
    # Instructs the model to detect language, transcribe, and translate to English.
    prompt = """Analyze the following audio clip. Identify the language being spoken, transcribe the content accurately, and then translate the entire transcription into English. Provide *only* the final English transcription as plain text, without any additional explanations, labels, or formatting."""



    # Generate content by passing the file directly
    with open(file_path, 'rb') as f:
        audio_file = genai.upload_file(path=file_path) # Preferred way for larger files potentially

    # Send the prompt and the uploaded file reference to the model
...
def extract_ehr_components(transcript: str) -> dict:
    ...
    try:
        ...
        ehr_data = json.loads(response.text.strip())
        return ehr_data
        ...
        return {"error": "Failed to parse structured data", "rawText": response.text.strip()}
...
def process_audio_for_ehr(file_path: str) -> dict:
    ...
    # Print the structured EHR data in the terminal
    print("\nExtracted EHR Data:")
...
if __name__ == "__main__":
    ...
    file_path = os.path.join(os.path.dirname(__file__), "media", "sample2.mp3")
    
    if not os.path.exists(file_path):
        print(f"Error: Audio file not found at {file_path}")
        # Try alternative paths
        alt_path = "automated-testing/src/media/sample2.mp3"
